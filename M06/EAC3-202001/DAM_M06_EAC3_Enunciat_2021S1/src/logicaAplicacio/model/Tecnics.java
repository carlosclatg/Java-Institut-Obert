...
@NamedQueries({
        @NamedQuery(
                name = "Tecnics.getAll",
                query = "SELECT i FROM Tecnics i"),
        @NamedQuery(
                name="Tecnics.getByEspecialitat",
                query= "SELECT i FROM Tecnics i WHERE i.especialitat = :especialitat"
        ),
        @NamedQuery(
                name="Tecnics.getBySalari",
                query= "SELECT i FROM Tecnics i WHERE i.salari = :salari"
        )

})
public class Tecnics extends Professor {

    @Column(columnDefinition = "especialitat", nullable = false)
